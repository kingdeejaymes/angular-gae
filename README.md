# Angular-Google-Auth Boilerplate

An opinionated kickstarter Boilerplare for Angular application integrated with Google authentication and Appengine Environment
This is a fork from existing boilerplate here # [ngBoilerplate](http://joshdmiller.github.com/ng-boilerplate)

***
### Goal of this modified boilerplate
- The goal is to make life *easier and simpler* for the developer.
- To quickly jump in to the development of business related stories instead configuring build tools,
setting up environment, setting up most used libraries and congifuring it.
- This is to remove repetitive development task thats always being used on all project and do the coding immediately.

***

### Key Features and additional changes from the ngbp boilerplate:
- Login module added for authentication with *Google sign-in*
- Main Routing modified to only allow authenticated users to access specific page.
- Auto injection of Google-ID-token on headers on each request to Backend Enabled Oath2.0 authentication
- Deployment to Appengine and Running to local script added for ease of use
- Configurations of each environment added and automated by scripts
- Unit test is changed to headless Chrome instead of Firefox
- app.yaml added for appengine deployment and static file mapping
- Angular Material dependencies added.
- Ui-Bootstrap latest library added.
- MomentJS dependencies added.
- Bootstrap and Font Awesome updated and seamlessly working.
- JQuery Added.

so you just have to code and build your app :)

***

### Quick Start
Install System dependencies that are out of npm:
```sh
...
and then from the application root directory, Install this Boilerplate
```sh
...
Configuration for local enviroment is located under 'src/config/local.txt'
This will compile and build the codes in "build" directory

### Deployment
```sh
$ deploy.sh <environment> <version>
```
Configuration for appengine enviroment is located under 'src/config/< dev | qa | stating | prod >.txt'
No need to do 'grunt' manually. the deploy script will execute it automatically.
This will compile, build and minift the codes in "bin" directory and will be uploaded to appengine.

### Adding new third-party library via Bower
vendor/ - third-party libraries. Bower will install packages here.
Anything added to this directory will need to be manually added to
build.config.js and karma/karma-unit.js to be picked up by the build system
